...
export default function BuildCommands({
	build = { clear: false }, instance, name, style = { }
}: BuildCommandProps) {

	// State
	const [ commands, commandsSet ] = useState<string>('');
	...
	useEffect(() => {
		...
		if(instance.backups && build.backup) {
			lCommands.push(
				`mv ${instance.web_root} ${instance.backups}/${iso(new Date(), true, false, true)}`
			)
		}
	...
	// Render
	return <Box style={style}><pre>{commands}</pre></Box>
}
